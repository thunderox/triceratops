...
JCRev :: JCRev(MY_FLOAT T60, int sample_rate)
{
  ...
  if (sample_rate < 44100.0) {
    double srscale = sample_rate / 44100.0;
    for (i=0; i<9; i++)	{
  ...
  for (i=0; i<4; i++)
	{
	  ...
	  combCoeff[i] = pow(10,(-3 * lens[i] / (T60 * sample_rate)));
...
{
  if (std::isnan(input) || input < -1 || input > 1) input = 0;
